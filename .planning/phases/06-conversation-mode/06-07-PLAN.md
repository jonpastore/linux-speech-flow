...
must_haves:
  truths:
    ...
    - "Settings window has a 'Conversation Mode' section with: API keys (Grok, Gemini), conv_save_dir, conv_feedback_mode, conv_max_qa_iterations, conv_auto_analyze toggle, default prompt textarea, and a qualifying questions list editor (Gtk.TextView, one per line, saved as list to config['conv_qualifying_questions'])"
...
# Qualifying questions list editor
...
# Default qualifying questions covering audience, complexity, and purpose
_default_qq = [
...
# Save qualifying questions as a JSON list (split on newlines, strip blanks)
import json as _json
_qq_raw = self._conv_qq_buf.get_text(
    self._conv_qq_buf.get_start_iter(),
    self._conv_qq_buf.get_end_iter(), False
)
self._config["conv_qualifying_questions"] = [
...
"`
  ...
  <done>
Settings has "Conversation Mode" section with Grok/Gemini API key fields (PasswordEntry), save location, feedback mode combo, max Q&A spin, auto-analyze checkbox, default prompt textarea, and qualifying questions editor (Gtk.TextView, one question per line). All values save to config on save button click; qualifying questions saved as list to config["conv_qualifying_questions"].
  </done>
</task>
