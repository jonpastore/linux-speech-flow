...
class HotkeyManager:
    ...
    def __init__(
        ...
        on_conversation_stop=None,
        on_conversation_feedback_toggle=None,
        on_huddle_start=None,
        on_huddle_stop=None,
    ):
    ...
    def _reload_bindings_from_config(self) -> None:
        ...
        self._bindings = {
            action: parse_combo(
                config.get(HOTKEY_CONFIG_KEYS[action], HOTKEY_DEFAULTS[action])
            )
    ...
    def _on_press(self, key):
        ...
        if self._matches_binding(key, "record"):
            if self._state == self._STATE_IDLE:
                ...
                logger.debug("hotkey record: ignored (state=%s)", self._state)
        elif (
            self._matches_binding(key, "stop") and self._state == self._STATE_RECORDING
        ):
            GLib.idle_add(self._stop_recording, False)
            ...
            if self._state == self._STATE_IDLE:
                ...
                logger.debug("hotkey conversation: ignored (state=%s)", self._state)
        elif self._matches_binding(key, "reprocess"):
            GLib.idle_add(self._on_reprocess)
        elif (
            self._matches_binding(key, "feedback")
            and self._state == self._STATE_CONVERSATION
        ):
            ...
            if self._state == self._STATE_IDLE:
                ...
                logger.debug("hotkey huddle: ignored (state=%s)", self._state)
    ...
    def _start_recording(self) -> bool:
        ...
        logger.info(
            "start recording sounds_enabled=%s device=%r", sounds_enabled, output_device
        )
